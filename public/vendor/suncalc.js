...
// Tried that first; reverted after finding it's not actually stable across
// MM core versions: MM 2.36 ships suncalc v1.9.0 via js/vendor.js's
// "suncalc.js" -> "node_modules/suncalc/suncalc.js" (a direct, classic-
// script serve of whatever the npm package's own build is) - v1.9.0
// returns altitude in RADIANS, no conversion. MM 2.37+ ships v2.x via a
// different bridge (js/vendor.js -> js/suncalc-global.mjs, an ESM import of
// the same npm package) - v2.x returns DEGREES, a breaking change from v1.
// Same vendor.js key, same module code, silently different units depending
// entirely on which MM core version happens to be installed - not safe to
// depend on. Also, independently, MM core's own default `clock`/`weather`
// modules load whichever of those MM core ships as `window.SunCalc` too -
// so even ignoring the version issue, another script tag on the page can
// still overwrite window.SunCalc out from under this module regardless of
