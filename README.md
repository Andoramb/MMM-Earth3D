...
| `rotationSpeed`          | number | `20`    | Spin speed, `0` (stopped) to `25` (fastest, ~144s/revolution) - values above `25` are accepted but clamped to the same speed as `25`, since the full `0`-`100` range felt too fast well before reaching `100`. Always spins around the globe's own polar axis, so it stays correct even when `camera.rotate` tilts the globe. |
