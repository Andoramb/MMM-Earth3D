...
// Owns the Flight layer's OpenSky polling lifecycle for node_helper.js: given live `flights` config, polls states/all, matches the flight, pushes PLANET3D_FLIGHT_POSITION; flights.* is session state, not persisted (except the credentials file).
...
function createFlightTracker(options) {
	...
	function sendPosition(payload) {
		sendSocketNotification("PLANET3D_FLIGHT_POSITION", payload);
	}
	...
	async function poll() {
		...
		try {
			...
			Log.error("[MMM-Planet3D node_helper] flight poll failed (" + consecutiveFailures + " consecutive): " + status.lastError);
		} finally {
			polling = false;
		}
	}
	...
	// Called on every PLANET3D_FLIGHTS_STATE push (see MMM-Planet3D.js's sendFlightsState()).
...
function loadCredentials(file) {
	try {
		...
		Log.warn("[MMM-Planet3D node_helper] could not read flight-credentials.json (" + err.message + ") - starting with no OpenSky credentials configured");
		return null;
	}
}
